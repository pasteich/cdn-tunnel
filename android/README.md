...
```
- Доступ даёт либо мастер-ключ владельца (`-password`), либо удостоверение
  ссылки (`-cred`). Сервер отказал → **403/409**, клиент печатает `STATUS …`
  (`authfail`, `linkgone`, `linkrevoked`, `linkused`, `linkbad`) и выходит,
  VPN не поднимается. Если владелец удалил или отозвал ссылку **уже после**
  подключения, клиент узнаёт об этом на очередном пинге (раз в 15 с), и сервис
  гасит VPN — телефон не остаётся с мёртвым туннелем вместо интернета.
...
## Вкладки
**Туннель · Сервер · Трафик** (журнал — внизу вкладки «Трафик»).
...
Вкладка **Туннель**: поле для ссылки `cdn://…` + кнопки «Импортировать» /
«Из буфера», строка с текущим сервером, и свёрнутая **«Ручная настройка»**:
IP сервера CDN, Host/SNI, мастер-ключ (нужен только владельцу), локальный порт
SOCKS, conns, **Транспорт (chunked/stream)**, **Метод (post/get)**, переключатель
UDP; в «Дополнительно» — DNS, MTU, **Fast Open**, блок IPv6, debug-лог.
...
Ссылка **не содержит пароля**: внутри — удостоверение, подписанное мастер-ключом
...
Каждая выпущенная ссылка попадает в реестр на сервере, и **реестр — это и есть
список доступа**: пускают только ссылки, которые в нём есть. В панели видно
**все** ссылки — даже те, которыми ещё не воспользовались, — с их трафиком,
устройством и состоянием. «Удалить» закрывает доступ навсегда, «Отозвать» —
до кнопки «Вернуть».

Вкладка **Сервер** → **«Управление сервером»** открывает панель владельца
(подключается по SSH и читает `/admin` у запущенного сервера): аптайм, сколько
ссылок на линии, трафик сервера и **по каждой ссылке (↓/↑ за всё время)** —
состояние, устройство, адрес. Действия по ссылке: «Поделиться», «Сбросить»,
«Отвязать», «Отозвать/Вернуть», «Удалить»; плюс перезапуск и обновление сервера.

Вкладка **Трафик** показывает ↓/↑ (всего и скорость), UDP, соединения и
**пинг (RTT)**.
...
1. поля: адрес VPS, SSH-порт, пользователь (root), SSH-пароль; `-addr` (по умолч. `:80`),
   метод сервера (`both`/`post`/`get`), «рестарт сервиса каждые N минут» (0 = не рестартить).
   Лимита клиентов нет: сколько ссылок выпущено — столько людей и подключится;
